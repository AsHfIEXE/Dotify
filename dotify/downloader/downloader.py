...
def _require_dependency(path: str | None, name: str) -> None:
    """Raise DotifyDependencyNotFound if a required binary is missing."""
    if not path:
        raise DotifyDependencyNotFound(name)
...
class SpotifyDownloader:
    ...
    async def _download(self, item: DownloadItem) -> None:
        if self.synced_lyrics_only:
            raise DotifySyncedLyricsOnly()

        if item.final_path and Path(item.final_path).exists() and not self.overwrite:
            raise DotifyMediaFileExists(item.final_path)

        if item.media.tags.media_type in {
            MediaType.SONG,
            MediaType.PODCAST,
        }:
            if self.audio.download_mode == AudioDownloadMode.ARIA2C:
                _require_dependency(self.base.aria2c_full_path, "aria2c")

            if self.audio.download_mode == AudioDownloadMode.CURL:
                _require_dependency(self.base.curl_full_path, "cURL")
            ...
            if (
                ...
                _require_dependency(self.base.ffmpeg_full_path, "ffmpeg")

            if (
                item.media.stream_info.audio_track.file_format == "mp4"
                and self.audio.remux_mode == AudioRemuxMode.MP4BOX
            ):
                _require_dependency(self.base.mp4box_full_path, "MP4Box")

            if (
                item.media.stream_info.audio_track.file_format == "mp4"
                and self.audio.remux_mode in {AudioRemuxMode.MP4DECRYPT, AudioRemuxMode.MP4BOX}
            ):
                _require_dependency(self.base.mp4decrypt_full_path, "mp4decrypt")
            ...
            if self.video.remux_mode == VideoRemuxMode.FFMPEG:
                _require_dependency(self.base.ffmpeg_full_path, "ffmpeg")

            if self.video.remux_mode == VideoRemuxMode.MP4BOX:
                _require_dependency(self.base.mp4box_full_path, "MP4Box")
            ...
            if item.media.decryption_key:
                ...
                if (
                    ...
                    _require_dependency(self.base.shaka_packager_full_path, "Shaka Packager")
