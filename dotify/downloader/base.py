...
class SpotifyBaseDownloader:
    ...
    def sanitize_string(
        ...
        if file_ext is None:
            ...
            if self.truncate is not None:
                max_len = max(1, self.truncate - len(file_ext))
                sanitized_string = sanitized_string[:max_len]
    ...
    def _apply_ogg_tags(
        ...
        if not skip_tagging and "cover" not in exclude_tags and cover_bytes:
            picture = self._create_picture(cover_bytes)
    ...
    @staticmethod
    def _create_picture(cover_bytes: bytes) -> Picture:
        """Create a FLAC/Vorbis Picture from raw cover bytes."""
        picture = Picture()
