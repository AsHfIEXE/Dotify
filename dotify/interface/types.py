...
class MediaTags:
    ...
    def _format_date(self, date_format: str | None = None) -> str | None:
        """Format the date field for tag output."""
        if isinstance(self.date, datetime.date):
            return self.date.isoformat() if date_format is None else self.date.strftime(date_format)
    ...
    def as_mp4_tags(self, date_format: str = None) -> dict[str, list[Any]]:
        ...
        date_mp4 = self._format_date(date_format)
    ...
    def as_vorbis_tags(self, date_format: str = None) -> dict[str, list[Any]]:
        date_flac = self._format_date(date_format)
