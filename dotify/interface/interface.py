...
class SpotifyInterface:
    ...
    async def _get_track_media(
        ...
        if track_data["__typename"] != "Track":
            base_media.error = DotifyMediaNotFoundException(track_id)
            return base_media

        if not track_data["playability"]["playable"]:
            base_media.error = DotifyMediaUnstreamableException(track_id)
            return base_media
        ...
        if self.flat_filter:
            ...
            if flat_filter_result:
                base_media.error = DotifyMediaFlatFilterException(
                    track_id,
                    flat_filter_result,
                )
                return base_media
    ...
    async def _get_episode_media(
        ...
        if episode_data["__typename"] != "Episode":
            base_media.error = DotifyMediaNotFoundException(episode_id)
            return base_media

        if not episode_data["playability"]["playable"]:
            base_media.error = DotifyMediaUnstreamableException(episode_id)
            return base_media
    ...
    async def _get_album_media(
        ...
        if album_data["__typename"] != "Album":
            yield SpotifyMedia(
                ...
                error=DotifyMediaNotFoundException(media_id),
            )
        else:
        ...
        while total is None or offset < total:
            ...
            if liked_tracks_data["__typename"] != "UserLibraryTrackPage":
                yield SpotifyMedia(
                    ...
                    error=DotifyMediaNotFoundException("liked-tracks"),
                )
    ...
    async def get_media(
        ...
        if auto_media_option == AutoMediaOption.LIKED_TRACKS:
            ...
            if not url_info or url_info.media_type in self.base.disallowed_media_types:
                ...
                raise DotifyUnsupportedMediaTypeException(media_type)
