...
from ..api import DotifyRequestException
...
class SpotifySongInterface(SpotifyAudioInterface):
    ...
    async def proccess_media(
        ...
        media.cover_url = (
            self.parse_cover_url(media.album_metadata["coverArt"]["sources"][0]["url"])
            if media.album_metadata["coverArt"]["sources"]
            else None
        )
    ...
    async def get_lyrics(self, track_id: str) -> MediaLyrics | None:
        ...
        except DotifyRequestException as e:
